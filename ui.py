...
for(;shown<d.results.length;shown++){const x=d.results[shown],inv=x.invoice||{},e=document.createElement('article');e.className='result';const h=document.createElement('h3');h.textContent=inv.invoice_number||x.invoice_path;const b=document.createElement('span');b.className='badge '+x.outcome;b.textContent=x.outcome.replaceAll('_',' ');const p=document.createElement('p');p.className='meta';p.textContent=[inv.vendor,inv.amount,inv.currency].filter(Boolean).join(' · ');const reason=document.createElement('p');reason.textContent=x.recovery||x.error||(x.validation_issues||[]).join('; ')||((x.processing.approval||{}).reason||'Completed');e.append(h,b,p,reason);q('results').append(e)}
