...
| High-value demonstration invoice | Exercise the required additional-scrutiny route in the normal bundled batch; every high-value assessment invoice fails inventory before approval. | `data/invoices/invoice_vp_review.txt` is an explicitly labeled InvoiceFlow test case with valid inventory and a USD 12,500 total. The default mock VP leaves it pending. |
| Local browser UI | Let an operator choose a directory and see results without waiting for the entire batch. | `ui.py` binds only to localhost, defaults to the bundled invoices, publishes each completed result, and links to the final audit report. A missing key or Grok transport failure activates offline recovery with a visible notice. Fixed offline fixtures cannot interpret new documents, which remain clear per-file errors while the batch continues. CLI mode selection remains explicit. |
